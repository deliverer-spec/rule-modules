...
import com.ciana.rule.engine.node.data.DataUtil;
...
public class DataExecute implements ExecInterface{
	...
	public Map<String, Object> initParams(RuleNode node, Map<String, Object> lastParams, Boolean isTest) throws Exception {
		...
		Map<String,Object> tmpdata = DataUtil.getInstanceClass(node.getDatatype()).getData(node, lastParams);
		//转换来源
		...
		for(Data data: datas) {
			...
			if(tmpdata.containsKey(colum)) {
				...
				lastParams.put(colum, tmpdata.get(colum));
			}else {
				//TODO 没有确定默认值的使用方式
